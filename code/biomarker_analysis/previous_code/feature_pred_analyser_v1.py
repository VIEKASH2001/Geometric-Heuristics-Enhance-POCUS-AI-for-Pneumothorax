...
def getModelFeaturePreds(dataset_split_dict, exp_path, exp_name, label_dict_path, train_folds, test_folds, TRAIN = False, multi_task = False, use_probs = False):
    ...
    for idx, video in enumerate(video_list):
        ...
        if use_probs:
            ...
            # pred = (torch.sigmoid(torch.tensor(prob_preds[pred_idx])).mean(0) > 0.5).numpy().astype(int)
            pred = (torch.sigmoid(torch.tensor(prob_preds[pred_idx])).max(0)[0] > 0.5).numpy().astype(int)
            # pred = preds[pred_idx].mean(0).astype(int)
