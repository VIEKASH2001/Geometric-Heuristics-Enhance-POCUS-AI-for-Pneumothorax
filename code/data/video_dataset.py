...
class LSULungDataset(Dataset):
    ...
    def _temporalSampling(self, frames_list, sf_idx):
        ...
        if self.temp_sampling_method == "equi_temp_sampling":
            ...
            clip_frame_ids = np.arange(start_frame, len(frames_list), clip_stride).astype(int)
            ...
            clip_frame_ids = np.arange(start_frame, start_frame+(self.clip_width*clip_stride), clip_stride).astype(int)
    ...
    def mls_rigid_deformation_inv(self, clip, p, q, alpha=1.0, density=1.0):
        ...
        for i in range(t):
            ...
            transformed_image = image[tuple(transformers.astype(int16))]    # [grow, gcol]
    ...
    def getVideosFromSplit(self):
        ...
        [video_list.extend(self.split_dict[f"{fold}"]) for fold in self.foldsToIncludeList]
